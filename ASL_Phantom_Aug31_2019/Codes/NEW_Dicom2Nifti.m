...
% codepath = pwd;
% cd ../
% dicompath = fullfile(pwd,'Dicom','SUB01','20190901.Techdev_Ph_Sept1_Experiments_Set1.19.09.01_13_18_40_DST_1.3.12.2.1107.5.2.43.167024');
% 
...
for sb = 1:length(A)    % Loop over the number of Subjects
    ...
    for i =1:length(pld_msec)
        ...
        aslsesdir = dir(fullfile(Niftipath,A{sb},strcat('3DSp_ubpCASL_TR6s_LD',num2str(LD),'_PLD',num2str(pld_msec(i)),'_BS0_PUMP',num2str(pump),'_ASL*')));
         m0sesdir = dir(fullfile(Niftipath,A{sb},strcat('3DSp_ubpCASL_TR6s_NoLabel_BS0_PUMP',num2str(pump),'_ASL*')));   % using Unlabeled ASL images as M0 for each pump rate.
