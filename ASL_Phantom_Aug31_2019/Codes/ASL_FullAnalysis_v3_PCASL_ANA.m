...
unit_factor = 6000;   %%%  Flow unit conversion from mL/Vvox/sec into mL/100mg/min)
...
ROIflag = 2;   %%%  1 = Sphere Segment ROI, 2 = Full Phantom, 3 = Jet ROI 

PCASL_exp = ["ASYM", "GmaxGavg4", "FA20"];
PCASLflag = 3;  % Select the experiment  from above list 1 = "ASYM", 2 = "GmaxGavg4", 3 = "FA20"
PCASL_exp(PCASLflag)

dset = 2 ;    % Two PCASL datasets are available
...
%%%  Read in an original nii volume. 
  ...
  firstfile = 's2019-08-31_12-33-124254-00001-00001-1.nii';
...
%%%%%%   PCASL Experiments    %%%%%%%%
		  ...
		  aslsesdir = dir(fullfile(Niftipath,A{sb},strcat('PCASL_Experiments_Set',num2str(dset),'/3DSp_ubpCASL_TR6s_LD',num2str(asl.LD),'_PLD',num2str(asl.pld_msec(i)),'_',PCASL_exp(PCASLflag),'_PUMP',num2str(pump(pp)),'_ASL*'))); 
           m0sesdir = dir(fullfile(Niftipath,A{sb},strcat('PCASL_Experiments_Set',num2str(dset),'/3DSp_ubpCASL_TR6s_LD',num2str(asl.LD),'_PLD',num2str(asl.pld_msec(i)),'_',PCASL_exp(PCASLflag),'_PUMP',num2str(pump(pp)),'_M0*')));
...
if PCASLflag == 1
       ...
       v0.fname = fullfile(strcat('par_maps/results/PCASL_Exp_Set',num2str(dset),'/NoSmooth/'),strcat('PCASL_ASYM_Perf_LD',num2str(asl.LD),'_PLD',num2str(asl.pld_msec(iip)),'.nii'));         
       spm_write_vol( v0,squeeze(weighted_perf(:,:,:,iip)) );

elseif PCASLflag == 2 
       v0.fname = fullfile(strcat('par_maps/results/PCASL_Exp_Set',num2str(dset),'/NoSmooth/'),strcat('PCASL_GmaxGavg4_Perf_LD',num2str(asl.LD),'_PLD',num2str(asl.pld_msec(iip)),'.nii'));         
       spm_write_vol( v0,squeeze(weighted_perf(:,:,:,iip)) );
       
elseif PCASLflag == 3
       v0.fname = fullfile(strcat('par_maps/results/PCASL_Exp_Set',num2str(dset),'/NoSmooth/'),strcat('PCASL_FA20_Perf_LD',num2str(asl.LD),'_PLD',num2str(asl.pld_msec(iip)),'.nii'));         
       spm_write_vol( v0,squeeze(weighted_perf(:,:,:,iip)) );
...
if ~ismember(ROIflag,[1 2 3])
    sprintf("Please select correct ROIflag: 1 = Sphere Segment ROI, 2 = Full Phantom, 3 = Jet ROI !")
    return
elseif ROIflag == 1  
...
%%%%      NO SMOOTHING, No Weighting  %%%%
   ...
   if PCASLflag == 1
          v0.fname = fullfile(strcat('par_maps/results/PCASL_Exp_Set',num2str(dset),'/NoSmooth/'),...
                  strcat('CALC_ASYM_CBF_LD',num2str(asl.LD),'_PLD',num2str(asl.pld_msec(iip)),'.nii'));         
          spm_write_vol( v0,squeeze(Calc_CBF(:,:,:,iip)) );
          
    elseif PCASLflag == 2
          v0.fname = fullfile(strcat('par_maps/results/PCASL_Exp_Set',num2str(dset),'/NoSmooth/'),...
                  strcat('CALC_GmaxGavg4_CBF_LD',num2str(asl.LD),'_PLD',num2str(asl.pld_msec(iip)),'.nii'));         
          spm_write_vol( v0,squeeze(Calc_CBF(:,:,:,iip)) );
        
    elseif PCASLflag == 3
          v0.fname = fullfile(strcat('par_maps/results/PCASL_Exp_Set',num2str(dset),'/NoSmooth/'),...
                  strcat('CALC_FA20_CBF_LD',num2str(asl.LD),'_PLD',num2str(asl.pld_msec(iip)),'.nii'));         
          spm_write_vol( v0,squeeze(Calc_CBF(:,:,:,iip)) );
...
figure(5);
  ...
  sig = eval(sprintf('sig_pump%d', pump(pp)));
   ...
   [ff, gof]= fit((asl.pld+asl.tau)',(sig)',ft,'StartPoint',[1.0,0.02],'problem',{asl.lambda,asl.tau,asl.T1blood});
   gof
...
%   
 ...
 %% Calculated flow from CBF Maps : CONSENSUS paper
  ...
  % mean_CBF
  eval(sprintf('ROI%d_mean_CBF_pump%d = [mean_CBF]',ROIflag, pump(pp)))         % FULL Phantom  
...
%   title('3 mm Phantom, BS = 0','FontSize',16);
 ...
 hold off
   ...
   %%%  1 = Sphere Segment ROI, 2 = Full Phantom, 3 = Jet ROI 
    if ROIflag == 1
        fname=fullfile(codepath,strcat('par_maps/results/FitResults/Experiments/Set',num2str(dset),'/OVERLAY_',PCASL_exp(PCASLflag),'_SpherCon_Perf_vs_PLD_R',num2str(R),'The',num2str(angle),'Slice',num2str(slice),'OzMax',num2str(Oz_max)));
        print(strcat(fname),'-djpeg');  
        
    elseif ROIflag == 2
        fname=fullfile(codepath,strcat('par_maps/results/FitResults/Experiments/Set',num2str(dset),'/OVERLAY_',PCASL_exp(PCASLflag),'_FULLPhantom_Perf_vs_PLD'));
        print(strcat(fname),'-djpeg'); 
        
    elseif ROIflag == 3
        fname=fullfile(codepath,strcat('par_maps/results/FitResults/Experiments/Set',num2str(dset),'/OVERLAY_',PCASL_exp(PCASLflag),'_JET_ROI_Perf_vs_PLD'));
        print(strcat(fname),'-djpeg');  
