function [meanperf,M0,meanCBF,weight] = ReturnTriplet(aslsesdir,M0sesdir,subid,pld_val,asl,GeomMask)
...
%%%%%  Control - Label with step size = 2; (2:end) means go through entire volume   
       ...
       % perf=vpa(perf,10);
        ...
        var_perf = var(perf,1,4);    % variance between 100 repetitions        
        std_perf = std(perf,1,4);    % Standard Deviation between 100 repetitions
        stderr_perf =  std_perf ./ sqrt(50);
        weight = 1./var_perf;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 %%%% Read mearged 4D volumes: M0 
        ...
        var_M0 = var(spm_read_vols(spm_vol(P)),1,4);    % variance between 2 repetitions        
        std_M0 = std(spm_read_vols(spm_vol(P)),1,4);    % Standard Deviation between 2 repetitions
        stderr_M0 = std_M0 ./ sqrt(2);     
...
% 
%  figure(10)   
%  set(gcf, 'Position',  [600, 900, 900, 550]);
%   edges = -5:0.1:5;
%   hh= perf .* GeomMask;
%  % size(hh);       
% 
...
% 
%  % title ('Mean Perf \Delta M ');
%   xlabel (' \Delta M  avg-ed within GeaomMask ');
%                      
%   fname=fullfile(aslpath,strcat('hist_GeomMaskMean_Perf_dM_PLD', num2str(pld_val)));
%   print(strcat(fname),'-djpeg');  
%   
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 
%  figure(11)  
%  set(gcf, 'Position',  [1400, 900, 900, 550]);
%  edges = -30:1.:70;
%  m =  meanperf .* GeomMask;
% 
%  msk = m~=0;
...
% 
%   title ('Mean Perf \Delta M ');
%   xlabel ('Mean \Delta M  avg-ed in GeaomMask for time series');
%                      
%   fname=fullfile(aslpath,strcat('hist_meanPerf_GeomMask_Voxels_PLD', num2str(pld_val)));
%   print(strcat(fname),'-djpeg');  
%   
%   
%   
...
%%%%%  Only divide to non-zero M0 masked region, to avoid dividing by 0. 
        ...
        meanCBF(msk) = CBF_factor .* (meanperf(msk)./(10*M0(msk)));   % mL/100g/min
       % meanCBF(msk) = (meanperf(msk)./M0(msk));  
        ...
        spm_write_vol(v0,meanperf .* GeomMask);
        
%         v0.fname = fullfile(aslpath,['meanCBF_' subid '.nii']);
%          % v0.dt = [16 0];  %  DT_FLOAT   16   /* float (32 bits/voxel) */ from NIFTI header
%         spm_write_vol(v0,meanCBF .* GeomMask);
%              
        v0.fname = fullfile(aslpath,['Variance_Perf_' subid '.nii']);
        spm_write_vol(v0,var_perf .* GeomMask);
        
        v0.fname = fullfile(aslpath,['Weights_' subid '.nii']);   
        spm_write_vol(v0,weight .* GeomMask);
        
        v0.fname = fullfile(aslpath,['STD_Perf_' subid '.nii']);
        spm_write_vol(v0,std_perf .* GeomMask);
        
        
        v0.fname = fullfile(aslpath,['STDERR_Perf_' subid '.nii']);
        spm_write_vol(v0,stderr_perf .* GeomMask);
        ...
        msk2 =  ratio > 0.05; 
       % size(msk2)
        rstderrper_perf = stderr_perf./meanperf;
        rstderrper_perf(isnan(rstderrper_perf))=0; 
        rstderrper_perf(isinf(rstderrper_perf))=0; 
       
        v0.fname = fullfile(aslpath,['meanPerf_5pct_' subid '.nii']);   
        spm_write_vol(v0,meanperf .* msk2 .* GeomMask);
        
        v0.fname = fullfile(aslpath,['RATIO_STDERRPerf_Perf_' subid '.nii']);
        spm_write_vol(v0,rstderrper_perf .* GeomMask);
        
        v0.fname = fullfile(aslpath,['RATIO_STDERRPerf_Perf_5pct_' subid '.nii']);
        spm_write_vol(v0,(rstderrper_perf .* msk2) .* GeomMask);
        
        
        v0.fname = fullfile(aslpath,['LOG10_RATIO_STDERRPerf_Perf_5pct_' subid '.nii']);
        spm_write_vol(v0,log10((rstderrper_perf .* msk2) .* GeomMask));
        
        
        
      %  z-score = meanperf
           z_score_perf = meanperf ./ stderr_perf ;
           z_score_perf(isnan(z_score_perf))=0; 
           z_score_perf(isinf(z_score_perf))=0; 
        
        v0.fname = fullfile(aslpath,['z_score_perf_5pct_' subid '.nii']);
        spm_write_vol(v0, (z_score_perf .* msk2) .* GeomMask);
        
        
        v0.fname = fullfile(aslpath,['STDERR_Perf_5pct_' subid '.nii']);
        spm_write_vol(v0,stderr_perf.* msk2 .* GeomMask);
        ...
        v0.fname = fullfile(aslpath,['meanM0_Masked' subid '.nii']);
        spm_write_vol(v0,M0.* GeomMask);
         
        v0.fname = fullfile(aslpath,['Variance_M0_' subid '.nii']);
        spm_write_vol(v0,var_M0 .* GeomMask);
        
        v0.fname = fullfile(aslpath,['STD_M0_' subid '.nii']);
        spm_write_vol(v0,std_M0 .* GeomMask);

        v0.fname = fullfile(aslpath,['STDERR_M0_' subid '.nii']);
        spm_write_vol(v0,stderr_M0 .* GeomMask);
        
        v0.fname = fullfile(aslpath,['STDERR_M0_5pct_' subid '.nii']);
        spm_write_vol(v0,stderr_M0 .* msk2 .* GeomMask);
         
                    
        rstderrM0_M0 = stderr_M0./M0;
        rstderrM0_M0(isnan(rstderrM0_M0))=0; 
        rstderrM0_M0(isinf(rstderrM0_M0))=0; 
      
        v0.fname = fullfile(aslpath,['RATIO_STDERRM0_M0' subid '.nii']);
        spm_write_vol(v0,rstderrM0_M0 .* GeomMask);
        
        
         v0.fname = fullfile(aslpath,['RATIO_STDERRM0_M0_5pct_' subid '.nii']);
        spm_write_vol(v0,rstderrM0_M0.* msk2 .* GeomMask);
