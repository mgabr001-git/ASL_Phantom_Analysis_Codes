...
%
% 1.  Convert to Nifti: Use NEW_Dicom2Nifti.m
...
ROIflag = 1;  %%%  1 = Spherical Segment ROI, 2 = Full Phantom, 3 = Jet ROI 
...
%%% Sphere Segment Reference Points
...
%%%  Read in an original nii volume. 
 ...
 v0.fname = fullfile(strcat(roipath,'/SphericalSegmentROI_R',num2str(R),'_Slice',num2str(slice),'_OzMax',num2str(Oz_max),'.nii'));  
...
%%%%%%  as ROI mask with respect to Geometric Markers.
        ...
        [meanperf, M0, weight] = ReturnTriplet(aslsesdir,m0sesdir,subid,asl.pld_msec(i),asl,GeomMask);
...
figure(5);
  ...
  sig = eval(sprintf('sig_pump%d', pump(pp)));
   ...
   [ff, gof]= fit((asl.pld+asl.tau)',(sig)',ft,'StartPoint',[0.3,0.05],'problem',{asl.lambda,asl.tau,asl.T1blood});
   gof
...
%   title('3 mm Phantom, BS = 0','FontSize',16);
 ...
 hold off
   ...
   if ROIflag == 1
        fname=fullfile(codepath,strcat('par_maps/results/FitResults/2M0_Analysis/NoSmooth/OVERLAY_SphericalSegmentROI_Perf_vs_PLD_R',num2str(R),'Slice',num2str(slice),'OzMax',num2str(Oz_max)));
